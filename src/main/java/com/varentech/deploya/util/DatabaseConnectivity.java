...
public class DatabaseConnectivity {
    ...
    public static void findDatabase() {
        ...
        logg.info("Database name: " + config.getString("varentech.project.path_to_database"));
    ...
    public static void findDataBase(String path){
        ...
        logg.info("Loading values from the config file located at " + path);
        String pathOfDataBase = config.getString("varentech.project.path_to_database");
        try{
            ...
            logg.info("Using default values to create a database.");
            findDatabase();
        }
    }
    ...
    public static void databaseCheck() throws SQLException {
        ...
        if (resultSet.next()){
            logg.info("Entries table exists");
            ...
            logg.info("Entries table does not exist, creating Entries table...");
        ...
        if(resultSet.next()){
            logg.info("Entries_Details table exists");
            ...
            if(resultSet.next()){logg.info("entries_id column exists");}
            ...
            if(resultSet.next()){logg.info("hash_value column exists");}
            resultSet = metaData.getColumns(null, null, "Entries_Details", "output");
            tempColName = "output";
            if(resultSet.next()){logg.info("output column column exists");}
            resultSet = metaData.getColumns(null, null, "Entries_Details", "error");
            tempColName = "error";
            if (resultSet.next()){logg.info("error column exists");}
            else{
            ...
            logg.info("Entries_Details table does not exits, creating Entries_Details table...");
            EntriesDetailsDaoImpl impl = new EntriesDetailsDaoImpl();
